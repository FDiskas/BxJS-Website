...
const searchWorker = typeof window === "object" && new (SearchWorker as any)()

const getResults = async (input: string) => {
  const results = await searchWorker.search(input)
  return results
}

function Search() {
  const [results, setResults] = useState<SearchWorker.SearchResults>([])
